...
class ServiceGenerator extends BaseGenerator
{
    ...
    {
        $templateData = view('laravel-generator::services.index', $this->variables())->render();

        g_filesystem()->createFile($this->path . '/' . $this->serviceFileName, $templateData);

        $this->config->commandComment(infy_nl().'Service : ' . $this->config->modelNames->name . 'Service.php');
