...
class FrontQuasarGenerator extends BaseGenerator
{
    ...
    {
        $templateData = view('laravel-generator::front.page', $this->variables())->render();

        g_filesystem()->createFile($this->path . '/src/pages/registers/' . $this->pageFileName, $templateData);

        $this->config->commandComment(infy_nl().'Page : ' . $this->pageFileName);
        $this->config->commandInfo($this->pageFileName);
    }

    protected function generateGrid()
    {
        $templateData = view('laravel-generator::front.grid', $this->variables())->render();

        g_filesystem()->createFile($this->path . '/src/components/registers/' . $this->gridFileName, $templateData);

        $this->config->commandComment(infy_nl().'Grid : ' . $this->gridFileName);
        $this->config->commandInfo($this->gridFileName);
    }

    protected function generateForm()
    {
        $templateData = view('laravel-generator::front.form', $this->variables())->render();

        g_filesystem()->createFile($this->path . '/src/components/forms/' . $this->formFileName, $templateData);

        $this->config->commandComment(infy_nl().'Form : ' . $this->formFileName);
        $this->config->commandInfo($this->formFileName);
    }

    protected function generateModel()
    {
        $templateData = view('laravel-generator::front.model', $this->variables())->render();

        g_filesystem()->createFile($this->path . '/src/models/' . $this->modelFileName, $templateData);

        $this->config->commandComment(infy_nl().'Model : ' . $this->modelFileName);
    ...
    {
        return [
            'fieldOptions'       => implode(','.infy_nl_tab(1, 2), $this->generateFields()),
            'columns'             => implode(','.infy_nl_tab(1, 2), $this->generateGridColumn()),
            'rowData'             => implode(','.infy_nl_tab(1, 2), $this->generateRowData()),
            'fieldsForm'             => implode(''.infy_nl_tab(1, 0), $this->generateFieldForm()),
            'fieldsModel'            => implode(';'.infy_nl(1).infy_tab( 2), $this->generateFieldsModel()) . ',',
